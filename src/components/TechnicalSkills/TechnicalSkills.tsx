    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get("https://curriculum-data-api.onrender.com/get/technical_skills?people_id=" + personId)
            ...
            <Card title={
                    <>
                        ...
                        <Card type="card" isLoading={isLoading} classes={`${infoFocused ? styles.active : styles.inactive} ${styles.info} bg-light-grey`} content={
                            <h5>Habilidades técnicas são capacidades ou aptidões que um profissional possui. A barra de aptidão mensura o nível de confiança, experiência prática e conhecimentos específicos.</h5>
                        }>
                        </Card> 
                    </>
